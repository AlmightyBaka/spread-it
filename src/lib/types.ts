...
export type DocumentType<T> = T extends SheetType.Csv
	? undefined
	: T extends SheetType.Excel
	? Workbook
	: T extends SheetType.GoogleSheets
	? GoogleSpreadsheet
	: never

export type SettingsType<T> = T extends SheetType.Csv
	? SettingsCsv | SettingsCsvFile
	: T extends SheetType.Excel
	? SettingsExcel | SettingsExcelFile
	: T extends SheetType.GoogleSheets
	? SettingsGoogleSheets
	: never

export type OutputType<T> = T extends SheetType.Csv
	? OutputCsv
	: T extends SheetType.Excel
	? OutputExcel
	: T extends SheetType.GoogleSheets
...
export type SettingsExcel = HasSheets &
	HasHeader &
	HasHeaderStyle &
	HasColumnWidth
export type SettingsExcelFile = HasSheets &
	HasHeader &
	HasHeaderStyle &
	HasColumnWidth &
	HasFile

export type SettingsGoogleSheets = HasSheets &
	HasHeader &
	HasHeaderStyle &
	HasColumnWidth &
	HasShrink & {
...
export type DefaultSettings = HasSheets &
	HasHeader &
	HasHeaderStyle &
	HasShrink &
	HasColumnWidth
...
export type OutputClosure<T extends SheetType> = (
	data: object[],
	settings?: Settings,
) => Promise<OutputType<T>>
export type OutputClosureGoogleSheets = (
	data: object[],
	settings: Settings,
) => Promise<OutputType<SheetType.GoogleSheets>>
...
export interface IGoogleSheetsProcessor
	extends IDocumentProcessor<GoogleSpreadsheet> {
	createDocument(
		spreadsheetId: string,
		credentials: GoogleSheetsCredentials,
	): Promise<void>
