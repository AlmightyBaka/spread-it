...
describe('creating Excel document module', () => {
	...
	const testObjIncomplete: TestObj = {
	...
	test('should insert data', async () => {
		...
		expect(sheet.row(2).cell(3).value()).toBe(
			JSON.stringify(testObjIncomplete.a),
		)
	...
	test('should set header', async () => {
		...
		expect(sheet.row(3).cell(3).value()).toBe(
			JSON.stringify(testObjIncomplete.a),
		)
	})
	...
	test('should set header styles', async () => {
		...
		expect(
			sheet
				.row(1)
				.style(['bold', 'horizontalAlignment', 'verticalAlignment', 'fill']),
		).toStrictEqual({
			bold: true,
	...
	test('should set column width', async () => {
		...
		columnWidths.forEach((columnWidth) => {
			expect(sheet.column(columnWidth.index + 1).width()).toBe(
				columnWidth.width,
			)
		})
